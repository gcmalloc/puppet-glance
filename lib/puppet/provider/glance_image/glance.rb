...
Puppet::Type.type(:glance_image).provide(
  ...
  def create
    ...
    additional_args = []
    if resource[:min_disk] 
        additional_args.push("--min-disk=#{resource[:min_disk]}")
    end

    if resource[:min_ram] 
        additional_args.push("--min-ram=#{resource[:min_ram]}")
    end

    results = auth_glance('image-create', "--name=#{resource[:name]}", "--is-public=#{resource[:is_public]}", "--container-format=#{resource[:container_format]}", "--disk-format=#{resource[:disk_format]}", location, *additional_args)
    ...
    if id
      @property_hash = {
        ...
        :min_disk         => resource[:min_disk],
        :min_ram          => resource[:min_ram],
        :id               => id
      }
    else
  ...
  def minimum_ram=(value)
    auth_glance('image-update', id, "--min-ram=#{value}")
  end

  def minimum_disk=(value)
    auth_glance('image-update', id, "--min-disk=#{value}")
  end
